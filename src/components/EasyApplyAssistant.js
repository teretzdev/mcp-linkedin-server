...
const EasyApplyAssistant = ({ onRequestGeminiKey }) => {
  ...
  // Detect missing required fields whenever questions or applicantProfile changes
  useEffect(() => {
    if (!questions.length) return;
    const requiredFields = questions
      .map(q => mapQuestionToProfileField(q))
      .filter(Boolean);
    const missing = requiredFields.filter(field => !applicantProfile[field] || applicantProfile[field] === '');
    setMissingFields(missing);
  }, [questions, applicantProfile]);

  // Map question to profile field name
  const mapQuestionToProfileField = (question) => {
    // Map common question categories to profile fields
    switch (question.category) {
      case 'authorization': return 'work_authorization';
      case 'compensation': return 'salary_expectation';
  ...
  const handleSaveMissingFields = async (e) => {
    ...
    try {
      ...
      setApplicantProfile(prev => ({ ...prev, ...missingFieldValues }));
      setMissingFields([]);
      setMissingFieldValues({});
      // Automatically re-trigger answer generation for unanswered questions
      setTimeout(() => {
        questions.forEach(q => {
          const field = mapQuestionToProfileField(q);
          if (field && (!applicantProfile[field] || applicantProfile[field] === '')) {
            // Now filled, so generate answer if not already answered
            if (!answers[q.id]) {
              generateAnswer(q.id, q);
            }
          }
        });
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setProfile(data);
        // Use profile to pre-fill applicantProfile
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {missingFields.length > 0 && (
          <form onSubmit={handleSaveMissingFields} className="mb-6 bg-yellow-50 p-4 rounded-lg">
            <h3 className="font-semibold text-yellow-800 mb-2">We need a bit more info to complete your application:</h3>
